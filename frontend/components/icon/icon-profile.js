export default function IconSearch({ className }) {
  return (
    <svg className={className} width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path
        d="M2.75 19.7083L2.87381 18.9124C3.08239 17.5716 3.95679 16.4218 5.25648 16.0316C6.71006 15.5952 8.7892 15.125 11 15.125C13.2108 15.125 15.2899 15.5952 16.7435 16.0316C18.0432 16.4218 18.9176 17.5716 19.1262 18.9124L19.25 19.7083"
        stroke="#353535"
        strokeWidth="2"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path
        d="M11.0003 10.9999C13.5316 10.9999 15.5837 8.94789 15.5837 6.41659C15.5837 3.88528 13.5316 1.83325 11.0003 1.83325C8.46902 1.83325 6.41699 3.88528 6.41699 6.41659C6.41699 8.94789 8.46902 10.9999 11.0003 10.9999Z"
        stroke="#353535"
        strokeWidth="2"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
  );
}
