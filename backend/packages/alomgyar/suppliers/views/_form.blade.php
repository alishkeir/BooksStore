@isset($model)
<form action="{{route('suppliers.update', ['supplier' => $model])}}" method="POST" enctype="multipart/form-data" id="form">
    @method('PUT')
@else
<form action="{{route('suppliers.store')}}" method="POST" enctype="multipart/form-data" id="form">
@endisset
    @csrf
    <div class="row">
        <div class="col-lg-8">
        ...
        <div class="col-lg-4">
            <div class="card">
                ...
                <div class="card-body">
                    <div class="form-group row">
                        <div class="col-lg-12">
                            <label class="col-form-label font-weight-bold">Beszállítói jutalék (%)</label>
                            <input name="percent" id="percent" type="number" class="form-control @error('percent') border-danger @enderror" value="{{ !is_null(old('percent')) ? old('percent') : $model->percent ?? '' }}">
                        </div>
                    </div>
                </div>
                @if($model ?? false)
