                ...
                <table class="table table-striped">
                    ...
                    @foreach($model as $post)
                        <tr>
                            <td>{{ $post->id }}</td>
                            <td>{{ $post->title }}</td>
                            <td>@if( $post->status ?? false)<span class="badge badge-success">Látható</span>@endif</td>
                            <td>
                                <div class="list-icons">
                                    @can('posts.show')
                                    <a href="{{ route('posts.show', ['post' => $post]) }}" class="list-icons-document text-primary-600"><i class="icon-eye"></i></a>
                                    @endcan
                                    @can('posts.storing')
                                    <a href="{{ route('posts.edit', ['post' => $post]) }}" class="list-icons-document text-primary-600"><i class="icon-pencil7"></i></a>
                                    @endcan
                                    @can('posts.destroy')
                                        <form action="{{route('posts.destroy', ['post' => $post])}}" class="d-inline" method="POST" onsubmit="return confirm({{ __('messages.delete-confirm') }});">
                                            @csrf
                                            @method('DELETE')
