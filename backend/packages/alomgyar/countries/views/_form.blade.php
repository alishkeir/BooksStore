@isset($model)
<form action="{{route('countries.update', ['country' => $model])}}" method="POST" enctype="multipart/form-data" id="form">
    @method('PUT')
@else
<form action="{{route('countries.store')}}" method="POST" enctype="multipart/form-data" id="form">
@endisset
    @csrf
    <div class="row">
        <div class="col-lg-8">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="form-group row">
                        <div class="col-lg-12">
                            <label class="col-form-label font-weight-bold">Ország kód</label>
                            <input name="code" id="code" class="form-control @error('code') border-danger @enderror" value="{{ !is_null(old('code')) ? old('code') : $model->code ?? '' }}">
                        </div>
                    </div>
                    <div class="form-group row">
                        <div class="col-lg-12">
                            <label class="col-form-label font-weight-bold">Szállítási díj</label>
                            <input type="number" name="fee" id="fee" class="form-control @error('fee') border-danger @enderror" value="{{ !is_null(old('fee')) ? old('fee') : $model->fee ?? '' }}">
                        </div>
                    </div>
                </div>
            </div>
