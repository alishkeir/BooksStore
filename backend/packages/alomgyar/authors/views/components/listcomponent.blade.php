                ...
                <table class="table table-striped">
                    <thead>
                        <tr>
                            ...
                            <th>
                                <a href="javascript:;" wire:click.prevent="sortBy('cover')" role="button" class="text-default">
                                    
                                    @include('admin::partials._sort-icons', ['field' => 'cover'])
                                </a>
                            </th>
                            <th>
                    ...
                    @foreach($model as $author)
                        <tr>
                            <td>{{ $author->id }}</td>
                            <td><img style="height:40px;" src="/storage/{{ $author->cover }}"></td>
                            <td>{{ $author->title }}</td>
                            <td>@if( $author->status ?? false)<span class="badge badge-success">Látható</span>@endif</td>
                            <td>
                                <div class="list-icons">
                                    @can('authors.show')
                                    <a href="{{ route('authors.show', ['author' => $author]) }}" class="list-icons-document text-primary-600"><i class="icon-eye"></i></a>
                                    @endcan
                                    @can('authors.storing')
                                    <a href="{{ route('authors.edit', ['author' => $author]) }}" class="list-icons-document text-primary-600"><i class="icon-pencil7"></i></a>
                                    @endcan
                                    @can('authors.destroy')
                                        <form action="{{route('authors.destroy', ['author' => $author])}}" class="d-inline" method="POST" onsubmit="return confirm({{ __('messages.delete-confirm') }});">
                                            @csrf
                                            @method('DELETE')
