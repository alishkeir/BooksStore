    ...
    */
    ...
    private function PostToBarion($url, $data)
    {
        ...
        if ($this->UseBundledRootCertificates) {
            curl_setopt($ch, CURLOPT_CAINFO, implode(DIRECTORY_SEPARATOR, [dirname(__FILE__), 'ssl', 'cacert.pem']));

            if ($this->Environment == BarionEnvironment::Test) {
                curl_setopt($ch, CURLOPT_CAPATH, implode(DIRECTORY_SEPARATOR, [dirname(__FILE__), 'ssl', 'gd_bundle-g2.crt']));
            }
        }
