...
class AddressRepairOlcsoCommand extends Command
{
    ...
    protected $signature = 'sync:address-repair-olcso {page=0}';

    protected $description = 'Check old address fields and update the customers addresses (olcso)';
    ...
    {
        $this->customersOlcso = DB::table('customers')->select('id', 'old_id', 'email')->where('store', 1)->get();
        $this->countries = DB::table('countries')->select('id', 'code')->get();
        $this->page = $this->argument('page') ?? 0;
        $this->repairOlcsoCustomersAddresses();
    ...
    private function repairOlcsoCustomersAddresses()
    {
        $olds = DB::select(DB::raw(' SELECT COUNT(*) as count FROM old_olcso_user '));
        $total = $olds[0]->count / $this->take;

        for ($i = $this->page; $i <= $total; $i++) {
            $oldUserIDs = DB::table('old_olcso_user_field_value')->select('user_field_value_user_id')->distinct()->skip($this->take * $this->page)->take($this->take)->get();
            foreach ($oldUserIDs as $oldUserID) {
                $oldUserValues = DB::table('old_olcso_user_field_value')->where('user_field_value_user_id', $oldUserID->user_field_value_user_id)->get();
                $newCustomer = $this->customersOlcso->where('old_id', $oldUserID->user_field_value_user_id)->first();
                if (! $newCustomer) {
                    continue;
                }
                $shippingAddress = [
